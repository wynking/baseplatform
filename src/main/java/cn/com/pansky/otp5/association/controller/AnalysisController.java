...
//                yAxis.add(String.valueOf(map.get(key).size()));
    ...
    public ResultBean analysisMap(HttpServletRequest request){
        ResultBean rb = new ResultBean();
        try{
    ...
    public ResultBean analysisCustom(HttpServletRequest request){
        ResultBean rb = new ResultBean();
        try{
            Map params = WebTools.getParameterMap(request);
            if(!StringUtils.isEmpty(request.getParameter("address"))){
            ...
            for(EnterpriseBasicVO vo : vos){
              String areaCode = vo.getAddress();
